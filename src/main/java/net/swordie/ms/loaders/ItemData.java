...
//            }
    ...
    /**
     * Helper method to convert optionType int to readable name
     */
    private static String getOptionTypeName(int optionType) {
        // Based on ItemOptionType enum values
        switch (optionType) {
            case 0: return "AnyEquip";
            case 10: return "Weapon";
            case 11: return "AnyExceptWeapon";
            case 20: return "Accessory";
            case 40: return "ArmorExceptGlove";
            case 51: return "Hat";
            case 52: return "Top";
            case 53: return "Bottom";
            case 54: return "Glove";
            case 55: return "Shoes";
            default: return "Unknown(" + optionType + ")";
        }
    }
...
//    }
    ...
    private static void createFilteredOptions() {
        // 1. Define the Whitelist: Allow only Flat Stats (No %, No Special)
        // Note: "All Stat" lines in WZ data typically provide str/dex/int/luk simultaneously,
        // so they will naturally pass this check.
        Set<BaseStat> allowedStats = new HashSet<>(Arrays.asList(
                BaseStat.str,
                BaseStat.dex,
                BaseStat.inte,
                BaseStat.luk,
                ...
                BaseStat.mdd,
                BaseStat.cr,
                BaseStat.crDmg, // Maybe remove crit damage
                BaseStat.ied,
                BaseStat.asr,
                BaseStat.ter,
                BaseStat.eleAttrResistance,
                BaseStat.eva,
                BaseStat.stance,
                BaseStat.allStat,
                BaseStat.hpRecovery,
                BaseStat.mpRecovery,
                BaseStat.summonTimeR,
                BaseStat.buffTimeR,
                BaseStat.dotBuffTimeR,
                BaseStat.reduceCooltime,
                BaseStat.reduceCooltimeR,
                BaseStat.costhpR,
                BaseStat.costmpR,
                BaseStat.costForceR,
                BaseStat.mpconReduce

        ));

        equipCubeItemOptions = getItemOptions().values().stream()
                .filter(io -> {
                    // Filter 1: The option must actually provide stats
                    if (io.getStatValuesPerLevel() == null || io.getStatValuesPerLevel().isEmpty()) {
                        return false;
                    }

                    // Filter 2: The option should not have "Misc" values
                    // (Misc values are usually for active skills, probability effects, or face expressions)
                    if (io.getMiscValuesPerLevel() != null && !io.getMiscValuesPerLevel().isEmpty()) {
                        return false;
                    }

                    // Filter 3: Inspect every stat provided by this option
                    for (Map<BaseStat, Double> levelStats : io.getStatValuesPerLevel().values()) {
                        if (levelStats.isEmpty()) continue;

                        for (BaseStat stat : levelStats.keySet()) {
                            // If the option contains ANY stat that is not in our allowed list
                            // (e.g., if it has BaseStat.strR or BaseStat.bd), reject the entire option.
                    ...
                    // If we passed all checks, include this option
                    return true;
                })
                .collect(Collectors.toSet());
    }
