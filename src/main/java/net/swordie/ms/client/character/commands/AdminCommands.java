...
//            outPacket.encodeArr(new byte[Integer.parseInt(args[1])]);
    ...
    @Command(names = {"jobadv"}, requiredType = Admin)
    public static class JobAdv extends AdminCommand {
        ...
        public static void execute(Char chr, String[] args) {
            new Thread(() -> {
                ...
                sm.setLastActiveScriptType(ScriptType.Npc);
                ...
                try {
                    ...
                    if (nextJobs.isEmpty()) {
                        sm.sendSayOkay("You do not meet the requirements for any job advancement at this time.");
                        return;
                    }
                    ...
                    if (nextJobs.size() > 1) {
                        ...
                        for (int i = 0; i < nextJobs.size(); i++) {
                            String jobName = JobConstants.JobEnum.getJobById(nextJobs.get(i).shortValue()).name();
                    ...
                    boolean confirmed = sm.sendAskYesNo("Are you sure you want to advance to #b" + jobName + "#k?");
    ...
    @Command(names = {"setexp", "exprate"}, requiredType = Admin)
