...
public class JobConstants {
    ...
    public static List<Integer> getJobAdvancementOptions(int currentJob, int level) {
        ...
        // --- Beginner / Noblesse / Citizen / Legend / etc. to 1st Job (Level 10+) ---
        if (level >= 10) {
            if (currentJob == JobEnum.BEGINNER.getJobId()) {
                options.add(JobEnum.WARRIOR.getJobId());
                options.add(JobEnum.MAGICIAN.getJobId());
                ...
                options.add(JobEnum.BLAZE_WIZARD_1.getJobId());
                options.add(JobEnum.WIND_ARCHER_1.getJobId());
                options.add(JobEnum.NIGHT_WALKER_1.getJobId());
                options.add(JobEnum.THUNDER_BREAKER_1.getJobId());
                options.add(JobEnum.MIHILE_1.getJobId());
            } else if (currentJob == JobEnum.CITIZEN.getJobId()) {
                options.add(JobEnum.BATTLE_MAGE_1.getJobId());
                options.add(JobEnum.WILD_HUNTER_1.getJobId());
                options.add(JobEnum.MECHANIC_1.getJobId());
                options.add(JobEnum.BLASTER_1.getJobId());
                options.add(JobEnum.DEMON_SLAYER_1.getJobId()); // Often branch from 3001, but sometimes 3000
                options.add(JobEnum.DEMON_AVENGER_1.getJobId());
            } else if (currentJob == JobEnum.LEGEND.getJobId()) options.add(JobEnum.ARAN_1.getJobId());
            else if (currentJob == JobEnum.EVAN_NOOB.getJobId()) options.add(JobEnum.EVAN_1.getJobId());
            else if (currentJob == JobEnum.MERCEDES.getJobId()) options.add(JobEnum.MERCEDES_1.getJobId());
            else if (currentJob == JobEnum.PHANTOM.getJobId()) options.add(JobEnum.PHANTOM_1.getJobId());
            else if (currentJob == JobEnum.LUMINOUS.getJobId()) options.add(JobEnum.LUMINOUS_1.getJobId());
            else if (currentJob == JobEnum.SHADE.getJobId()) options.add(JobEnum.SHADE_1.getJobId());
            else if (currentJob == JobEnum.DEMON.getJobId()) { // 3001
                options.add(JobEnum.DEMON_SLAYER_1.getJobId());
                options.add(JobEnum.DEMON_AVENGER_1.getJobId());
            }
            else if (currentJob == JobEnum.XENON.getJobId()) options.add(JobEnum.XENON_1.getJobId());
            else if (currentJob == 4001) options.add(JobEnum.HAYATO_1.getJobId()); // Hayato Beginner
            else if (currentJob == 4002) options.add(JobEnum.KANNA_1.getJobId()); // Kanna Beginner
            else if (currentJob == JobEnum.KAISER.getJobId()) options.add(JobEnum.KAISER_1.getJobId());
            else if (currentJob == JobEnum.ANGELIC_BUSTER.getJobId()) options.add(JobEnum.ANGELIC_BUSTER_1.getJobId());
            else if (currentJob == JobEnum.CADENA.getJobId()) options.add(JobEnum.CADENA_1.getJobId());
            else if (currentJob == JobEnum.KAIN.getJobId()) options.add(JobEnum.KAIN_1.getJobId());
            else if (currentJob == 15000) options.add(JobEnum.ILLIUM_1.getJobId());
            else if (currentJob == 15001) options.add(JobEnum.ARK_1.getJobId());
            else if (currentJob == 15002) options.add(JobEnum.ADELE_1.getJobId());
            else if (currentJob == 16000) options.add(JobEnum.HO_YOUNG_1.getJobId());
            else if (currentJob == 16001) options.add(JobEnum.LARA_1.getJobId());
            else if (currentJob == 11000) options.add(JobEnum.BEAST_TAMER_1.getJobId());
            else if (currentJob == 14000) options.add(JobEnum.KINESIS_1.getJobId());
            else if (currentJob == 301) options.add(JobEnum.PATHFINDER_1.getJobId()); // Sometimes Pathfinder is its own beginner logic
        }

        // --- 1st Job -> 2nd Job (Level 30+) ---
        if (level >= 30) {
            // Explorers
            if (currentJob == JobEnum.WARRIOR.getJobId()) {
                options.add(JobEnum.FIGHTER.getJobId());
                options.add(JobEnum.PAGE.getJobId());
                options.add(JobEnum.SPEARMAN.getJobId());
            } else if (currentJob == JobEnum.MAGICIAN.getJobId()) {
                options.add(JobEnum.FP_WIZARD.getJobId());
                options.add(JobEnum.IL_WIZARD.getJobId());
                options.add(JobEnum.CLERIC.getJobId());
            } else if (currentJob == JobEnum.BOWMAN.getJobId()) {
                options.add(JobEnum.HUNTER.getJobId());
                options.add(JobEnum.CROSSBOWMAN.getJobId());
            } else if (currentJob == JobEnum.THIEF.getJobId()) {
                options.add(JobEnum.ASSASSIN.getJobId());
                options.add(JobEnum.BANDIT.getJobId());
            ...
            // Dual Blade Special Case: Blade Recruit (430) -> Blade Acolyte (431)
                ...
                // Cannoneer / Pathfinder / others
            else if (currentJob == JobEnum.PIRATE_CANNONEER.getJobId()) options.add(JobEnum.CANNONEER.getJobId());
            else if (currentJob == JobEnum.PATHFINDER_1.getJobId()) options.add(JobEnum.PATHFINDER_2.getJobId());

                // General linear progression (1st -> 2nd)
                // Checks if job is a 1st job (ends in 00 usually) and not one of the explorers handled above
            else if (!isExplorer(currentJob) && currentJob % 100 == 0) {
                // Heuristic: usually 1st job ID + 10 = 2nd job ID
                // E.g. 1100 -> 1110. 2100 -> 2110.
                // Exceptions: Evan (2210 -> 2212)
                if (isEvan(currentJob)) {
                    if (currentJob == JobEnum.EVAN_1.getJobId()) options.add(JobEnum.EVAN_2.getJobId());
                } else {
                    options.add(currentJob + 10);
                }
            }
        }

        // --- Dual Blade Special Cases (Level 20 & 45) ---
        // Thief (400) -> Blade Recruit (430) at Level 20+
        if (level >= 20 && currentJob == JobEnum.THIEF.getJobId()) {
            // We add this as an option. If level >= 30, user will see 410, 420, AND 430.
            options.add(JobEnum.BLADE_RECRUIT.getJobId());
        }
        // Blade Acolyte (431) -> Blade Specialist (432) at Level 45+
        if (level >= 45 && currentJob == JobEnum.BLADE_ACOLYTE.getJobId()) {
            options.add(JobEnum.BLADE_SPECIALIST.getJobId());
        }

        // --- 2nd -> 3rd Job (Level 60+) ---
        if (level >= 60) {
            // Dual Blade: Blade Specialist (432) -> Blade Lord (433)
            if (currentJob == JobEnum.BLADE_SPECIALIST.getJobId()) {
                options.add(JobEnum.BLADE_LORD.getJobId());
            }
            // Evan (2212 -> 2214)
            else if (currentJob == JobEnum.EVAN_2.getJobId()) {
                options.add(JobEnum.EVAN_3.getJobId());
            }
            // General 2nd -> 3rd
            else {
                // 2nd jobs usually end in 10 or 30 (e.g. 110, 530, 1110)
                // Target is usually +1 or +1 (110->111)
                // We need to be careful not to catch jobs that are already 3rd job
                int sub = currentJob % 10;
                if (sub == 0 && currentJob > 0) { // e.g. 110, 1110.
                    // Special case: Cannoneer 530 -> 531
                    // Pathfinder 330 -> 331
                    options.add(currentJob + 1);
                }
            }
        }

        // --- 3rd -> 4th Job (Level 100+) ---
        if (level >= 100) {
            // Dual Blade: Blade Lord (433) -> Blade Master (434)
            if (currentJob == JobEnum.BLADE_LORD.getJobId()) {
                options.add(JobEnum.BLADE_MASTER.getJobId());
            }
            // Evan (2214 -> 2217)
            else if (currentJob == JobEnum.EVAN_3.getJobId()) {
                options.add(JobEnum.EVAN_4.getJobId());
            }
            // General 3rd -> 4th
            else {
                int sub = currentJob % 10;
                if (sub == 1) { // e.g. 111, 1111
                    options.add(currentJob + 1);
                }
            }
        }

        // Filter out invalid IDs (if logic produced any non-existent jobs)
        List<Integer> validOptions = new ArrayList<>();
        for (int id : options) {
            if (JobEnum.getJobById((short)id) != null && id != currentJob) {
                validOptions.add(id);
            }
        }
        return validOptions;
